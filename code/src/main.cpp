...
#define KERNEL_EXEMPLE_2 "__kernel void prototype(__global const double *PORTEFEUILLE, \
                                                 __global const double *ALEA, \
                                                 __global int *TIRAGES, \
                                                 __global int *nb_actions, \
    					         __global int *horizon, \
                                                 __global int *nb_tirages) { \
                                      int i = get_global_id(0); \
				      double tmp; \
				      for (int j = 0; j < *nb_actions; j++) { \
				        tmp = PORTEFEUILLE[j]; \
				        for (int k = 0; k < *horizon; k++) { \
				          tmp = tmp + tmp * ALEA[(i*(*horizon))+()]; \
					} \
                                      } \
			  }"
...
void prototype()
{
  ...
  std::cout << NB_ACTIONS * NB_TIRAGES * T * sizeof(double) << std::endl;
  double *N = (double *) calloc(NB_ACTIONS * NB_TIRAGES * T, sizeof(double));
  ...
  // je met tout dans la command_queue
